...
public static class ConHostSpawner
{
        ...
        out int processId)
    {
        ...
        if (!File.Exists(ConHostPath))
        {
            Console.WriteLine($"[ConHostSpawner] mthost not found at: {ConHostPath}");
            return false;
        }
...
#pragma warning restore CA1416
    ...
    private static bool SpawnUnix(string args, out int processId)
    {
        ...
        try
        {
            ...
            Console.WriteLine($"[ConHostSpawner] Spawned mthost (PID: {processId})");
            return true;
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    Console.WriteLine($"[ConHostSpawner] Spawned mthost as user (PID: {processId}, Session: {sessionId})");
                    return true;
                }
                finally
                {
