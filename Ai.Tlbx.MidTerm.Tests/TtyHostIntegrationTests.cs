...
/// </summary>
public class TtyHostIntegrationTests : IAsyncLifetime
{
    private readonly ITestOutputHelper _output;
    private Process? _ttyHostProcess;
    ...
    public async Task DisposeAsync()
    {
        ...
        if (_ttyHostProcess is not null && !_ttyHostProcess.HasExited)
        {
            try
            {
                _ttyHostProcess.Kill();
                await _ttyHostProcess.WaitForExitAsync();
            }
            catch { }
            _ttyHostProcess.Dispose();
        }
    }
    ...
    {
        _output.WriteLine($"Looking for mmttyhost.exe at: {TtyHostPath}");
        Assert.True(File.Exists(TtyHostPath), $"mmttyhost.exe not found at {TtyHostPath}");
    }

    [Fact]
    public async Task TtyHost_Spawn_CreatesNamedPipe()
    {
        _sessionId = GenerateSessionId();
        var pipeName = $"mt-con-{_sessionId}";

        _output.WriteLine($"Spawning mmttyhost with session {_sessionId}");
        _ttyHostProcess = StartTtyHost(_sessionId);
    ...
    [Fact]
    public async Task TtyHost_Connect_GetInfo_ReturnsSessionInfo()
    {
        _sessionId = GenerateSessionId();
        var pipeName = $"mt-con-{_sessionId}";
        ...
        Assert.Equal(TtyHostMessageType.Info, msgType);

        var info = TtyHostProtocol.ParseInfo(payload.Span);
        Assert.NotNull(info);
    ...
    [Fact]
    public async Task TtyHost_ReceivesOutput_FromShell()
    {
        _sessionId = GenerateSessionId();
        var pipeName = $"mt-con-{_sessionId}";
        ...
        Assert.Equal(TtyHostMessageType.Info, infoResponse.Value.type);
        _output.WriteLine("GetInfo handshake complete");

        // Send Enter to force a prompt (pwsh takes several seconds to start)
        var enterMsg = TtyHostProtocol.CreateInputMessage(Encoding.UTF8.GetBytes("\r\n"));
        ...
        while (DateTime.UtcNow < deadline)
        {
            ...
            if (type == TtyHostMessageType.Output)
            {
    ...
    [Fact]
    public async Task TtyHost_SendInput_ReceivesEcho()
    {
        _sessionId = GenerateSessionId();
        var pipeName = $"mt-con-{_sessionId}";
        ...
        // GetInfo handshake
        await _pipe.WriteAsync(TtyHostProtocol.CreateInfoRequest());
        var infoResponse = await ReadMessageAsync(_pipe, TimeSpan.FromSeconds(5));
        Assert.NotNull(infoResponse);
        ...
        {
            await _pipe.WriteAsync(TtyHostProtocol.CreateGetBuffer());
            var bufferResp = await ReadMessageAsync(_pipe, TimeSpan.FromSeconds(2));

            if (bufferResp?.type == TtyHostMessageType.Buffer)
            {
        ...
        var inputMsg = TtyHostProtocol.CreateInputMessage(Encoding.UTF8.GetBytes(inputText));
        ...
        while (DateTime.UtcNow < deadline)
        {
            ...
            if (msg.Value.type == TtyHostMessageType.Output)
            {
    ...
    [Fact]
    public async Task TtyHost_WaitForPrompt_EventuallyReceivesOutput()
    {
    ...
    [Fact]
    public async Task TtyHost_GetBuffer_ReturnsAccumulatedOutput()
    {
        _sessionId = GenerateSessionId();
        var pipeName = $"mt-con-{_sessionId}";
        ...
        Assert.Equal(TtyHostMessageType.Buffer, bufferResponse.Value.type);
    ...
    private Process StartTtyHost(string sessionId, string shell = "cmd", bool redirectOutput = false)
    {
    ...
    private static async Task<(TtyHostMessageType type, Memory<byte> payload)?> ReadMessageAsync(
