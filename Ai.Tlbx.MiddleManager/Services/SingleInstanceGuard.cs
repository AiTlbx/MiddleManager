...
/// <summary>
/// Ensures only one instance of mm.exe runs system-wide.
/// Uses named mutex on Windows, file lock on Unix.
/// </summary>
public sealed class SingleInstanceGuard : IDisposable
{
    private const string MutexName = "Global\\MiddleManager";
    private const string PidFileName = "middlemanager.pid";
    ...
    private bool TryAcquireWindows(out string? existingInfo)
    {
        ...
        try
        {
            ...
            existingInfo = "Another mm.exe instance is already running";
            _mutex.Dispose();
            _mutex = null;
            return false;
        }
    ...
    private bool TryAcquireUnix(out string? existingInfo)
    {
        ...
        try
        {
            ...
            {
                existingInfo = $"Another mm.exe instance is already running (PID: {existingPid})";
                _pidFile.Dispose();
                _pidFile = null;
                return false;
            }
        ...
        {
            existingInfo = "Another mm.exe instance is already running (file locked)";
            return false;
        }
        catch (Exception ex)
        {
    ...
    private static string GetPidFilePath()
    {
        ...
        {
            return "/usr/local/var/run/middlemanager.pid";
        }

        return "/var/run/middlemanager.pid";
    }
