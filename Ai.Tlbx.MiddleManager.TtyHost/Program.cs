...
using Ai.Tlbx.MiddleManager.TtyHost.Ipc;
using Ai.Tlbx.MiddleManager.TtyHost.Pty;

namespace Ai.Tlbx.MiddleManager.TtyHost;
...
public static class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        ...
        {
            Console.WriteLine($"mmttyhost {Version}");
            return 0;
        }
        ...
        Log($"mmttyhost {Version} starting for session {config.SessionId}");
    ...
    {
        Console.WriteLine($"""
            mmttyhost {Version} - MiddleManager Console Host

            Usage: mmttyhost --session <id> [options]
            ...
            IPC (macOS/Linux):
              Listens on Unix socket: /tmp/mm-con-<session-id>.sock
            """);
    }

    private static readonly string ExceptionLogPath = Path.Combine(LogDir, $"mmttyhost-exceptions-{StartTimestamp}.log");
